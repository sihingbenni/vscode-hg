...
export class Repository implements IDisposable {
  ...
  constructor(private readonly repository: BaseRepository) {
    ...
    const onRelevantHgChange = filterEvent(onRelevantRepositoryChange, uri =>
      /\/\.hg\//.test(uri.path)
    );
  ...
  async stage(...resourceUris: Uri[]): Promise<void> {
    await this.run(Operation.Stage, async () => {
      ...
      if (missingAndAddedResources.length) {
        const relativePaths: string[] = missingAndAddedResources.map(r =>
          this.mapResourceToRepoRelativePath(r)
        );
      ...
      if (rollback.kind === 'commit') {
        ...
        if (dryRunDetails && dryRunDetails.commitDetails) {
          ...
          if (this.stagingGroup.resources.length && affectedFiles.length) {
            ...
            this.stage(
              ...previouslyCommmitedResourcesToStage.map(r => r.resourceUri)
            );
          }
        }
      }
    ...
    await this.run(Operation.Pull, async () => {
      try {
        ...
        if (
          ...
          e.hgErrorCode === HgErrorCodes.DefaultRepositoryNotConfigured
        ) {
      ...
      try {
        ...
        if (
          ...
          e.hgFilenames = e.hgFilenames.map(filename =>
            this.mapRepositoryRelativePathToWorkspaceRelativePath(filename)
          );
        }
        throw e;
      }
  ...
  private async unlocked<T>(): Promise<void> {
    ...
    while (
      attempt <= 10 &&
      (await existsLocal(path.join(this.repository.root, '.hg', 'index.lock')))
    ) {
